...
let poCode = {encodeMsg};


try {
  const codeUtil = require('@sex-pomelo/pomelo-coderx');
  poCode.encodeMsg = codeUtil.encodeMsg || encodeMsg;
} catch (e) {
  poCode.encodeMsg = encodeMsg;
}
...
class Message {
  ...
  static encode (id, type, compressRoute, route, msg, compressGzip){
    return poCode.encodeMsg(id, type, compressRoute, route, msg, compressGzip);
  }
...
/**
   ...
   * @return {Buffer}               encode result
   */
  function encodeMsg (id, type, compressRoute, route, msg, compressGzip){
